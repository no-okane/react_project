...
-----
### API
- api key : 고유한 문자열 키 발급받아 요청 시 함께 전송하는 방식, 서버통신 적합
- oauth : 사용자 대신 api에 접근 할 수 있는 권한을 안전하게 위임받는 방식, 직접 로그인하여 권한 허가하는 과정 필요 (소셜 로그인, 사용자 계정 정보 접근)
- no : 별도의 api key 필요 없이 주소만 있으면 누구나 접근 가능
- https
    - yes일 경우 안전한 웹 프로토콜 접근 방식 지원
- cors
    - no : 웹 브라우저 보안정책으로 다른 도메인에서 api를 호출할 시 제한 둠
    - unknown : api 제공업체가 cors 지원 여부 명시 안 하므로 테스트 필요
    - yes : 브라우저에서 직접 api 호출 가능, 프론트엔드 js 바로 사용 가능